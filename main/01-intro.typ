...
Add citations with `@` like @meteoswiss-opendata (stored in `/tail/bibliography.bib`).

Acronym terms like #gls("hes-so") expand on first use and abbreviate after #gls("hes-so"). Glossary items such as #gls("rust") can also be used to show their description as such: #gls-description("rust"). Acronyms and glossary entries auto-generate at the document’s end (defined in `/tail/glossary.typ`).
